...
function getAlbum(req,res){
    ...
    Album.findById(albumId).populate({path: 'artist'}).exec((err, album)=> {
        if(err){
            es.status(500).send({message: 'Error en la peticion'});
        }else {
            if(!album){
...
function getAlbums(req, res){
    ...
    if(!artistId){
        ...
        //sacar el album y ordenarlo por título
        var find = Album.find({}).sort('title');

    }else {
        //Sacar los albums de un artista concreto de la BD
        //sacar los albumes cuyo artist sea igual al artistID
        //ordenar por los albumes más viejos a los más nuevos
    ...
    Album.findByIdAndRemove(albumId,(err, albumRemoved)=> {
        if(err){
            ...
            if(!albumRemoved){
                ...
                //Se elimina la song del artista
...
function getImageFile(req, res){
    ...
    var path_file = './uploads/albums/'+ imagenFile;
